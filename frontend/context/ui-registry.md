...
| Badge | `ui/badge.tsx` | cva chip, `bg-primary/10 text-primary border-border`. Variants: `pill` (rounded-full, `text-body-base` — section eyebrows; Hero overrides to `bg-surface text-muted-foreground` + pulse dot) and `tag` (rounded-md, `text-body-sm` — project tech tags). |
